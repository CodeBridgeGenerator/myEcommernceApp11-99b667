...
const ProjectSideBarLayout = (props) => {
  ...
  const mapSubSubmenus = (subSubmenus = [], parentKey = "") => {
    if (!Array.isArray(subSubmenus) || subSubmenus.length === 0)
      return undefined;
  ...
  const matchesConfig = (cfg, ctx) => {
    ...
    const rolesOk =
      !cfg.roles ||
      cfg.roles.length === 0 ||
      (ctx.roleId && cfg.roles.includes(ctx.roleId));
    const positionsOk =
      !cfg.positions ||
    ...
    const userOk =
      !cfg.user || (ctx.userId && String(cfg.user) === String(ctx.userId));

    // additional filters (single ids)
    const companyOk =
      !cfg.company ||
      (ctx.companyId && String(cfg.company) === String(ctx.companyId));
    const branchOk =
      !cfg.branch ||
      (ctx.branchId && String(cfg.branch) === String(ctx.branchId));
    const deptOk =
      !cfg.department ||
      (ctx.departmentId && String(cfg.department) === String(ctx.departmentId));
    const sectionOk =
      !cfg.section ||
      (ctx.sectionId && String(cfg.section) === String(ctx.sectionId));
  ...
  const scoreConfig = (cfg, ctx) => {
    ...
    if (
      ...
      cfg.profiles.length > 0 &&
      ctx.profileId &&
      cfg.profiles.includes(ctx.profileId)
    )
      base = 300;
    else if (
      cfg.positions &&
      cfg.positions.length > 0 &&
      ctx.positionId &&
      cfg.positions.includes(ctx.positionId)
    )
      base = 200;
    else if (
      cfg.roles &&
    ...
    const fetchMenus = async () => {
      ...
      try {
        ...
        if (context.departmentId)
          orList.push({ department: context.departmentId });
